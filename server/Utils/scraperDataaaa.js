...
class HebrewSearchScraper {
    constructor(options = {}) {
        ...
        this.textLimit = options.textLimit || 3000;
    ...
    async performSearch(associationName, keywords) {
        try {
            ...
            const contentResults = await this.scrapeUrlsContent(Array.from(uniqueUrls));
...
main(("שירותי רווחה","חזון ישעיה רשת הומניטארית (ע~ר)")).catch(console.error);