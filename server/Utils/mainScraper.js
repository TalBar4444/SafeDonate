...
//const RobotsParser = require('robots-txt-parser'); // Import robots-txt-parser
...
class HebrewSearchScraper {
    constructor(options = {}) {
        ...
        //this.robotsParser = RobotsParser({ userAgent: this.getNextUserAgent() });       
    }
    ...
    //         await this.robotsParser.fetch(site + '/robots.txt');
    //         return this.robotsParser.canCrawl(url);
    //     } catch (error) {
    //         console.warn(`Failed to fetch robots.txt for ${url}:`, error);
    //         return false; // Default to false if robots.txt can't be fetched
    //     }
    // }

    async scrapeData(associationName, associationNumber, category) {
		...
		searchResults.forEach(results => {
			results.forEach(result => {
				if (
					...
					!result.link.includes('data.gov.il') &&
					!result.link.includes('www.gov.il') &&
                    !result.link.includes('gov.il') &&
        ...
        // const allowedUrls = [];
        // for (const url of this.uniqueLinks) {
        //     if (await this.isAllowedByRobots(url)) {
        //         allowedUrls.push(url);
        //     } else {
        //         console.log(`Skipping ${url} due to robots.txt restriction.`);
        //     }
        // }

		// Scrape content from each unique URL
		const contentResults = await this.scrapeUrlsContent(Array.from(this.uniqueLinks));
        //const contentResults = await this.scrapeUrlsContent(allowedUrls);
        const processedResults = await this.processResults(contentResults);
...
module.exports = HebrewSearchScraper;